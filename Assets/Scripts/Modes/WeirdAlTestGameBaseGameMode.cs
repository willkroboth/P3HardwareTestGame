...
using PinballClub.WeirdAlTestGame.Modes.Menu;
...
namespace PinballClub.WeirdAlTestGame.Modes
{
	...
	/// </summary>
    public class WeirdAlTestGameBaseGameMode : BaseGameMode
	{
		private WeirdAlTestGameButtonCombosMode buttonCombosMode;
		...
		//private FeatureTestMode homeMode;
		private ShotsTestingMode homeMode;
		private ShotsMode shotsMode;
		private IntroVideoMode introVideoMode;

		public WeirdAlTestGameBaseGameMode (P3Controller controller)
			: base(controller)
		{

			WeirdAlTestGameBallLauncher.Initialize(controller, data);
            ...
            gameAttributeManagerMode = new WeirdAlTestGameGameAttributeManagerMode (p3, Priority);
			...
			attractMode = new WeirdAlTestGameAttractMode (p3, Priorities.PRIORITY_ATTRACT, "Attract");
			buttonCombosMode = new WeirdAlTestGameButtonCombosMode (p3, WeirdAlTestGamePriorities.PRIORITY_BUTTON_COMBOS);
			//mechManagerMode = new WeirdAlTestGameMechManager (p3, Priorities.PRIORITY_MECH);

			hudMode = new HUDMode (p3, Priorities.PRIORITY_MECH);
			popupMode = new PopupMode (p3, Priorities.PRIORITY_MECH);
			//homeMode = new FeatureTestMode (p3, WeirdAlTestGamePriorities.PRIORITY_HOME, "FeatureTest");
			homeMode = new ShotsTestingMode(p3, WeirdAlTestGamePriorities.PRIORITY_HOME, "ShotsTesting");
			introVideoMode = new IntroVideoMode (p3, WeirdAlTestGamePriorities.PRIORITY_HOME, "IntroVideo");
        ...
        protected override void InitializeGameData()
		{
			...
			Multimorphic.P3App.Data.DataManager.gameName = "WeirdAlTestGame";
		}
    ...
    public override void mode_stopped ()
		...
		protected override void RegisterSelectors() {
			...
			selectorManagerMode.RegisterSelector(new ProfileSettingsSelectorMode(p3, WeirdAlTestGamePriorities.PRIORITY_PROFILEATTRIBUTESELECTOR), "ProfileSettingsEditor", "Prefabs/Framework/ProfileSettingsEditor");
			...
			selectorManagerMode.RegisterSelector(new ConfirmationSelectorMode(p3, WeirdAlTestGamePriorities.PRIORITY_CONFIRMATION), "ConfirmationDialog", "Prefabs/Framework/ConfirmationDialog", true);
			selectorManagerMode.RegisterSelector(new ProfileSelectorMode(p3, WeirdAlTestGamePriorities.PRIORITY_PROFILESELECTOR), "ProfileDialog", "Prefabs/Framework/ProfileDialog", true);
			selectorManagerMode.RegisterSelector(new HighScoreNameSelectorMode(p3, WeirdAlTestGamePriorities.PRIORITY_HIGH_SCORE_NAME_SELECTOR), "HighScoreNameEntry", "Prefabs/Framework/HighScoreNameEntry", true);
			selectorManagerMode.RegisterSelector(new WeirdAlTestGameEventNameTextEditorMode(p3, WeirdAlTestGamePriorities.PRIORITY_EVENT_NAME_EDITOR), "EventNameEditor", "Prefabs/Framework/EventNameEditor", true);
			selectorManagerMode.RegisterSelector(new WeirdAlTestGameProfileNameTextEditorMode(p3, WeirdAlTestGamePriorities.PRIORITY_EVENT_NAME_EDITOR), "ProfileNameEditor", "Prefabs/Framework/ProfileNameEditor", true);
