...
namespace PinballClub.WeirdAlTestGame.Modes
{
    ...
    public class HomeMode : SceneMode
	{
		...
		private WeirdAlTestGameGameMode modeSummaryPendingMode;
		...
		{
			//multiball = new Multiball (p3, WeirdAlTestGamePriorities.PRIORITY_MULTIBALL);
			rightRampCounterMode = new ShotCounter(p3, WeirdAlTestGamePriorities.PRIORITY_SHOT_COUNTERS, "Evt_RightRampHit", "Evt_RightRampInc");
			leftRampCounterMode = new ShotCounter(p3, WeirdAlTestGamePriorities.PRIORITY_SHOT_COUNTERS, "Evt_LeftRampHit", "Evt_LeftRampInc");

			ballStartMode = new BallStartMode (p3, WeirdAlTestGamePriorities.PRIORITY_HOME_UTILITIES);
			lanesMode = new LanesMode (p3, WeirdAlTestGamePriorities.PRIORITY_LANES);

			ballSaveMode = new BallSaveMode (p3, WeirdAlTestGamePriorities.PRIORITY_BALL_SAVE);
			respawnMode = new RespawnMode (p3, WeirdAlTestGamePriorities.PRIORITY_RESPAWN);
			movingTargetMode = new MovingTargetMode (p3, WeirdAlTestGamePriorities.PRIORITY_MOVING_TARGET);
			sideTargetMode = new SideTargetMode (p3, WeirdAlTestGamePriorities.PRIORITY_SIDE_TARGET);
		...
		public bool BallStartCompleteEventHandler(string eventName, object eventData)
		{
            ...
            if (p3.Switches["buttonLeft0"].IsActive())
				WeirdAlTestGameBallLauncher.launch (WeirdAlTestGameBallLauncher.VUK_LEFT, LaunchCallback);
			else
				WeirdAlTestGameBallLauncher.launch (WeirdAlTestGameBallLauncher.VUK_RIGHT, LaunchCallback);
			int ballSaveTime = data.GetGameAttributeValue("BallSaveTime").ToInt();
			PostModeEventToModes ("Evt_BallSaveStart", ballSaveTime);
			PostModeEventToModes ("Evt_BallSavePauseUntilGrid", 0);
			ballStarted = true;
		...
		{
			WeirdAlTestGameBallLauncher.delayed_launch();
			return SWITCH_STOP;
		}
