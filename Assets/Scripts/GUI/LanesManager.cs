	...
	public class LanesManager : P3Aware {
		...
		public override void Update () {
			...
			if (awardSoundDelayTimer > 0) {
				...
				if (awardSoundDelayTimer <= 0) {
					WeirdAlTestGameAudio.Instance.PlaySound3D(soundName, gameObject.transform);
				}
			}
			...
			if (glintDelayTimer > 0 && glintCounter > 0) {
				...
				if (glintDelayTimer <= 0) {
					WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneCompleteGlints", gameObject.transform);
			...
			//WeirdAlTestGameAudio.Instance.PlaySound3D("Lane_Return_Lit", GetSidePos ((bool)eventObject));
			//WeirdAlTestGameAudio.Instance.PlaySound3D("Lane_Enables", GetSidePos ((bool)eventObject));

		public void AnimateLanesRightEventHandler(string evtName, object evtData) {
			List<bool> lanes = (List<bool>)evtData;
			WeirdAlTestGameAudio.Instance.PlaySound3D("LaneShiftRight", gameObject.transform);
		...
		public void AnimateLanesLeftEventHandler(string evtName, object evtData) {
			WeirdAlTestGameAudio.Instance.PlaySound3D("LaneShiftLeft", gameObject.transform);
			List<bool> lanes = (List<bool>)evtData;
			for (int i=0; i<lanes.Count; i++) {
				//if (lanes[i])
		...
		public void LaneActivatedEventHandler(string evtName, object evtData) {
			...
			if (lane == 0)
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneLight_0", gameObject.transform);
			else if (lane == 1)
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneLight_1", gameObject.transform);
			else if (lane == 2)
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneLight_2", gameObject.transform);
			else 
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneLight_3", gameObject.transform);
		}

		public void LaneAlreadyActivatedEventHandler(string evtName, object evtData) {
			int lane = (int)evtData;
			if (lane == 0)
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneOutNull", gameObject.transform);
			else if (lane == 1)
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneReturnNull", gameObject.transform);
			else if (lane == 2)
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneReturnNull", gameObject.transform);
			else 
				WeirdAlTestGameAudio.Instance.PlaySound3D("FX_LaneOutNull", gameObject.transform);
