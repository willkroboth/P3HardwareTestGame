...
namespace PinballClub.WeirdAlTestGame.GUI {
	...
	public class MultiPowJackpot : MonoBehaviour {
		...
		private void PlayPowSound() {
			WeirdAlTestGameAudio.Instance.PlaySound3D(powSoundName, gameObject.transform);
			numPowSounds--;
			SetupPowSoundTimer();
		}
		
		private void PlayJackpot() {
			WeirdAlTestGameAudio.Instance.PlaySound3D(jackpotSoundName, gameObject.transform);
