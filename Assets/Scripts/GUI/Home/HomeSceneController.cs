...
namespace PinballClub.WeirdAlTestGame.GUI {
	public class HomeSceneController : WeirdAlTestGameSceneController {
		...
		public override void Update () {
			...
			if (VUKStageTimer > 0) {
				...
				if (VUKStageTimer <= 0) {
					WeirdAlTestGameAudio.Instance.PlaySound3D("VUKRampUp", this.GetSidePos(launchSide));
				}
			}
		}
		...
		public void ShowLightModeEventHandler(string eventName, object eventObject) {
			...
			WeirdAlTestGameAudio.Instance.PlaySound3D("TractorBeamBallToVUKs", gameObject.transform);
			VUKStageTimer = 1.5f;
		}
		...
		public void ModeSummaryEventHandler(string eventName, object eventObject) {
			WeirdAlTestGameAudio.Instance.PlaySound3D("ModeSummary", gameObject.transform);
		...
		{
			WeirdAlTestGameAudio.Instance.PlaySound3D("ModeSummaryClose", gameObject.transform);
