...
namespace PinballClub.WeirdAlTestGame.GUI {
	...
	public class Bonus : BonusBackgroundBase {
		...
		private void UpdateDisplay() {
			...
			if (LineIndex < (BonusData.NumItems() + 2)) {
				...
				if (LineIndex == BonusData.NumItems() + 1) {
                    WeirdAlTestGameAudio.Instance.ChangePlaylistByName("Bonus_Total");
				}
				
				// FX
				if (LineIndex == BonusData.NumItems()) {
					WeirdAlTestGameAudio.Instance.PlaySound3D("Bonus_X", gameObject.transform);
					Multimorphic.P3App.Logging.Logger.Log ("Bonus_X");
				}
				else if (LineIndex < BonusData.NumItems()) {
					WeirdAlTestGameAudio.Instance.PlaySound3D("Bonus_Dings", gameObject.transform);
				}
		...
		public void SetBonusDataEventHandler(string eventName, object eventData) {
			WeirdAlTestGameAudio.Instance.ChangePlaylistByName("Bonus_Start");
